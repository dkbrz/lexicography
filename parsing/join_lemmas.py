...
def main():
    ...
    files = [f for f in os.listdir(folder + '/' + lang_1 + '/' + lang_1 + '_lemma_pos') if '.txt' in f]
    files.sort(key=lambda x: int(re.findall('[0-9]+', x)[0]))
    train_file = '{}/{}_lemma_pos.txt'.format(folder, lang_1)

    print('Joining files...')
    for key, file in enumerate(files):
        f = '{}/{}'.format(folder + '/' + lang_1 + '/' + lang_1 + '_lemma_pos', file)
        print('Adding file ' + f + '...')
