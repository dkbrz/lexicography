...
def main():
    ...
    files = [f for f in os.listdir(folder + '/' + lang_1 + '/' + lang_1 + '_token') if '.txt' in f]
    files.sort(key=lambda x: int(re.findall('[0-9]+', x)[0]))
    train_file = '{}/{}_token.txt'.format(folder, lang_1)

    print('Joining files...')
    for key, file in enumerate(files):
        f = '{}/{}'.format(folder + '/' + lang_1 + '/' + lang_1 + '_token', file)
        print('Adding file ' + f + '...')
